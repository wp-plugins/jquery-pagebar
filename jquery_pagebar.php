...
/* There are some elements from Pagebar2 from Lutz Schr&ouml;er. Thanks for his plugin. */
	...
	if($max_page > 1 && strpos(TEMPLATEPATH, 'wptouch') === false) { 
		...
		if ($options['show_next_prev'] == "on")	{
			...
			if ($paged > 1) {
				...
				$pagenumber_prev  =  $paged - 1;
		
				if (is_search()) {
					$loc_href_prev = '<a href="' . $url .'/?s=' . urlencode(get_query_var('s')) . '&paged='. $pagenumber_prev . '" title="' . __('Newer Entries','jquerypagebar') . '" >&laquo;</a>';
				} else {
					$loc_href_prev = '<a href="' . $url . '/?paged=' . $pagenumber_prev . '" title="' . __('Newer Entries','jquerypagebar') . '" >&laquo;</a>';
				}	
			}
			...
			if ($paged != $max_page) {
				...
				$pagenumber_next = $paged + 1;
				
				if (is_search()) {
					$loc_href_next = '<a href="' . $url .'/?s=' . urlencode(get_query_var('s')) . '&paged='. $pagenumber_next . '" title="' . __('Older Entries','jquerypagebar') . '" >&raquo;</a>';
				} else {
					$loc_href_next = '<a href="' . $url . '/?paged=' . $pagenumber_next . '" title="' . __('Older Entries','jquerypagebar') . '" >&raquo;</a>';
				}
			}
		}
	?>
            ...
            <div class="postbox">
                ...
                <div class="inside inside_first">
                    ...
                    <table>
                        ...
                        <tr>
                         	<td colspan="2"><p><label for="show_next_prev"><input type="checkbox" name="show_next_prev" <?php if ($options['show_next_prev'] == "on") echo "checked"; ?> id="show_next_prev" /> <?php _e('Show links to the next or previous page arround the page numbers. You should activated this option for visitors who had deactivated javascript.','jquerypagebar') ?></label>
                            </p>
                            </td>
                         </tr>
                    </table>
                    ...
                    <p><label for="load_jquery_ui"><input type="checkbox" name="load_jquery_ui" <?php if ($options['load_jquery_ui'] == "on") echo "checked"; ?> id="load_jquery_ui" /> <?php _e('Load jQuery UI with Slider from the plugin dir. It is version 1.7.1.','jquerypagebar') ?></label><p>
                    <p><br/></p>
